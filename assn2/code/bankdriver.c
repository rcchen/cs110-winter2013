...
static int workerTransactionErrors[MAX_WORKERS] = {0};
...
CreateBank(int testRunNum, int numWorkers, unsigned int initSeed, int verbose)
{
  ...
  /*
   * The checking code of the testing harness makes the assumption that all 
   * transactions are commutative. This is true only if no commands fail due 
   * to insuffcient funds. In the worse case the random number generator 
   * could make every command take the max amount from a single account.
   * This initial amount should never fail due to insufficient funds.  
   */
  initialAmount = numCommands * (AccountAmount)maxTransactionSize;
  ...
  if (testfailurecode) { 
    actionControl |= ACTION_TEST_INSUFFICIENT_FUNDS;
  }
...
Worker(void *threadarg)
{
  ...
  while (1) {
    ...
    err = Transaction_DoAction(bank, &action, workerNum, fixedBankBalance);
  ...
  workerBalanceErrors[workerNum] = errorCounts[ERROR_BALANCE_MISMATCH];
  workerTransactionErrors[workerNum] = errorCounts[ERROR_INSUFFICIENT_FUNDS] + 	
    errorCounts[ERROR_ACCOUNT_NOT_FOUND];
...
TestBank(int testRunNumber, unsigned int initSeed, uint64_t totalTime)
{
  ...
  for (int i = 0; i < MAX_WORKERS; i++) {
    ...
    transactionErrors += workerTransactionErrors[i];
  } 
  ...
  if (transactionErrors && !testfailurecode) {                            
    fprintf(stderr,"%d transactions failed. Serious Error!\n", transactionErrors);
  }    
