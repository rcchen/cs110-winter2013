...
Action_GetNext(int workerNum, Action *action, int control)
{
  ...
  case 2: {
    ...
    if ((control & ACTION_TEST_INSUFFICIENT_FUNDS) 
	&& (action->cmd == ACTION_WITHDRAW) && 
	((GetRandom(workerNum,0,0)%8) == 0)) { 
      /* We test insufficient funds by changing random 
       * withdrawals to use a too big amount */
      action->u.depwithArg.amount = INT64_MAX;
    }
    if (control & ACTION_NO_FUNDS_FLOW) {
      /* Change amount to zero if we were told to not withdraw or 
       * deposit funds */
  ...
  case 5: {
    ...
    if ((control & ACTION_TEST_INSUFFICIENT_FUNDS)
	&& ((GetRandom(workerNum,0,0)%8) == 0)) { 
      /* Check failure case by randomly making amount too big to work. */
      action->u.transArg.amount = INT64_MAX;
    }
    break;
  }
  case 6: {
