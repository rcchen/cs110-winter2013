/*
 * disksim - Simulate a disk for the assignment.  Your solution needs to work with this file unmodified. 
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
/* We want this function to show up in perf / gprof, so mark it as
 * NEVER_INLINE. */
static NEVER_INLINE
void SimulateDiskLatency(int64_t startTime)
{
  ...
  if (waitTimeSoFar < diskLatency) {
    ...
    if ((waitTime >= 10000) && !diskBusyWaitEnable) {
      ...
      while (Debug_GetTimeInMicrosecs() < endTime) {
        // spin
      }
    }
 ...
 * unsuccessful  */
int
disksim_open (char *pathname, int readOnly)
{
 ...
 */
int
disksim_readsector(int fd, int sectorNum, void *buf)
{
 ...
 */
int
disksim_close(int fd)
{
  return close(fd);
}

void
disksim_dumpstats(FILE *file)
{
