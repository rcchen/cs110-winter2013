...
void RemovePathname(struct unixfilesystem *fs, char *pathname);
void PrintUsageAndExit(char *progname);
int GetDirEntries(struct unixfilesystem *fs, int inumber, struct direntv6 *entries, int maxNumEntries);
...
void
DumpPathnameChecksum(struct unixfilesystem *fs, FILE *f) 
{
  DumpPathAndChildren(fs, "/", ROOT_INUMBER, f);
}

/*
 * Remove the specified pathame from the file system.
 */

void
RemovePathname(struct unixfilesystem *fs, char *pathname)
{
  /* Implement this for extra credit */
  (void) fs; /* avoid warning about unused parameter */
  fprintf(stderr, "RemovePathname(%s) unimplemented.\n", pathname);    
}
...
PrintUsageAndExit(char *progname)
{
  ...
  fprintf(stderr, "-r <pathname>    remove the specified pathname from the disk\n");
  exit(EXIT_FAILURE);
}
